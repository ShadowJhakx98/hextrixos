...
class NeuralNetworkVisualization:
    ...
    def resize(self, width, height):
        ...
        if current_time - self.last_resize_time > self.resize_cooldown:
            ...
            width_change = abs(self.width - width) / self.width if self.width > 0 else 1
            height_change = abs(self.height - height) / self.height if self.height > 0 else 1
    ...
    def draw(self, cr, pulse_value):
        ...
        Args:
            cr: Cairo context
            pulse_value: Animation pulse value
        """
        # Get the actual drawing area size
        allocation = cr.get_target().get_width(), cr.get_target().get_height()
        
        # Only resize if dimensions changed significantly
        width_change = abs(self.width - allocation[0]) / self.width if self.width > 0 else 1
        height_change = abs(self.height - allocation[1]) / self.height if self.height > 0 else 1
        
        if width_change > 0.01 or height_change > 0.01:
            current_time = time.time()
            if current_time - self.last_resize_time > self.resize_cooldown:
                self.resize(allocation[0], allocation[1])
                self.last_resize_time = current_time
